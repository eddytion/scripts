...
my @threads;
my @a_vioses = `sshpass -p 1234 ssh -q -l hscroot $hmc "for m in \\\$(lssyscfg -r sys -F name); do for l in \\\$(lssyscfg -r lpar -m \\\$m -F name,lpar_env | grep vioserver | egrep 'vsa|vsb' | cut -f1 -d,); do echo \\\$l; done; done"`;
chomp(@a_vioses);
for my $i (@a_vioses)
{
