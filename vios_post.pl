...
sub copy_files_locally {
  ...
  system("cp /mnt/vscsimap_$hostname /tmp/prev_config >/dev/null 2>&1");
}
...
sub fcs_tuning {
  ...
  foreach $fcs (@a_fcs)
  {
    ...
    {
      system("rmdev -l $fcs -R");
      system("chdev -l $fcs -a max_xfer_size=\"0x400000\"");
      system("cfgmgr -l $fcs");
    }
  }  
}
...
sub roce_config {
  ...
  if(scalar @a_roce_hba > 0 && scalar @a_roce_ent > 0)
  {
	...
	system("chdev -l $hba -a stack_type=ofed");
	system("cfgmgr");
      }
    }
  }
...
sub mkuser {
  ...
  my $bteamp = `echo asd | openssl enc -base64 -d`;
  chomp($unixp,$bteamp);
  system("mkgroup id=5000 bteam");
  system("mkuser id=5000 pgrp=bteam  home=/home/bteam shell=/usr/bin/ksh gecos=\"Bucharest L2 Team\" bteam");
  system("mkdir -p /home/bteam/.ssh; mkdir -p /opt/nagios/libexec");
  system("echo \"bteam:$bteamp\" | chpasswd");
  system("mkuser unix");
  system("echo \"unix:$unixp\" | chpasswd");
  system("pwdadm -c unix; pwdadm -c bteam");
  ...
  system("cp /mnt/conf/profile /home/unix/.profile; chown unix /home/unix/.profile; chmod 600 /home/unix/.profile");
  system("cp /mnt/conf/authorized_keys /home/bteam/.ssh/authorized_keys");
  system("cd /; tar xf /mnt/conf/libexec.tar");
  system("chown -R bteam:bteam /home/bteam/.ssh; chown bteam:bteam /opt/nagios/libexec/*");
}
