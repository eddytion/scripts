...
function dhcp_entry()
{
	...
	then
		...
		echo "host ${HOST1} { filename \"/boot_suse/powerpc-ieee1275/core.elf\"; hardware ethernet ${MAC}; fixed-address ${IP1}; }" >> /etc/dhcp/dhcpd.conf
...
EOD
	...
	then
		...
		echo "host ${HOST1} { filename \"/boot_suse/powerpc-ieee1275/core.elf\"; hardware ethernet ${MAC}; fixed-address ${IP1}; }" >> /etc/dhcpd.conf
		systemctl restart dhcpd
EOD
	fi	
}
...
function get_grub()
{
	...
	if [[ ${OS} == "sles12sp3" ]]
        then
                ...
                then
                        ssh -T root@lsh35350rh << EOD
                        rm -f /var/lib/tftpboot/boot_suse/grub.cfg-01-`echo ${MAC_GRUB}`
                        cd /var/lib/tftpboot/boot_suse
                        ln -s ./powerpc-ieee1275/grub.cfg3 grub.cfg-01-`echo ${MAC_GRUB}`
EOD
                fi
                if [[ ${HMC} == "ishmc40" || ${HMC} == "ishmc41" ]]
                then
                        ssh -T unix@lsh35551le << EOD
                        rm -f /tftpboot/boot_suse/grub.cfg-01-`echo ${MAC_GRUB}`
                        cd /tftpboot/boot_suse
                        ln -s ./powerpc-ieee1275/grub.cfg3 grub.cfg-01-`echo ${MAC_GRUB}`
EOD
                fi
        fi
	...
	then
		if [[ ${HMC} == "ishmc31" || ${HMC} == "ishmc30" ]]
			...
			rm -f /var/lib/tftpboot/boot_suse/grub.cfg-01-`echo ${MAC_GRUB}`
			cd /var/lib/tftpboot/boot_suse
			ln -s ./powerpc-ieee1275/grub.cfg2 grub.cfg-01-`echo ${MAC_GRUB}`
EOD
		fi
		if [[ ${HMC} == "ishmc40" || ${HMC} == "ishmc41" ]]
        	then
...
EOD
	...
	if [[ ${OS} == "sles12sp1" ]]
        then
			...
			rm -f /var/lib/tftpboot/boot_suse/grub.cfg-01-`echo ${MAC_GRUB}`
			cd /var/lib/tftpboot/boot_suse
                        ln -s ./powerpc-ieee1275/grub.cfg1 grub.cfg-01-`echo ${MAC_GRUB}`
EOD
                fi
                if [[ ${HMC} == "ishmc40" || ${HMC} == "ishmc41" ]]
                then
