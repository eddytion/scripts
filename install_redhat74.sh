...
#	HOST1=$(echo ${LPAR}rh)
	...
	then
		...
		echo "host ${HOST1} { filename \"/boot_redhat74/powerpc-ieee1275/core.elf\"; hardware ethernet ${MAC}; fixed-address ${IP1}; }" >> /etc/dhcp/dhcpd.conf
...
EOD
	...
	then
		...
		echo "host ${HOST1} { filename \"/boot_redhat74/powerpc-ieee1275/core.elf\"; hardware ethernet ${MAC}; fixed-address ${IP1}; }" >> /etc/dhcpd.conf
		systemctl restart dhcpd
EOD
	fi	
}
