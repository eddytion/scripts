        ...
        if (mysqli_num_rows($result) > 0) 
        {
                ...
                else
                {
                  $class="odd";
                        ...
                        echo "<tr class=\"$class\" onmouseover=\"this.style.color='red'\" onmouseout=\"this.style.color='black'\">";
                        echo "<td>" . "<a class=\"text-primary\" href=\"https://{$row["hmc_id"]}\" target=\"_blank\">{$row["hmc_id"]}</a>" . "<br><a href=\"hmc.php?hmc={$row['hmc_id']}\" onclick=\"return popitup('hmc.php?hmc={$row['hmc_id']}')\">HMC Details</a>". "</td><td>" . 
                             ...
                             $row["msserial"] . "</td><td>" . 
                             "<i class=\"fas fa-desktop\"></i>&nbsp;" .$row["lparname"] . " <i class=\"fas fa-paste\" title=\"Click to copy mkvterm command to clipboard\" id=\"copy-button\" data-clipboard-text=\"mkvterm -m {$row['msname']} -p {$row['lparname']}\"></i>"
